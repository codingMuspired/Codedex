...
def main():
    ...
    # this is to track by how much the players' paddle will move per frame
    paddle_1_move = 0
    paddle_2_move = 0
    ...
    while True:
        ...
        60 is an arbitrary number but the game runs smooth at 60fps
        """
        ...
        we also multiple the `move` variable by the delta time to keep movement consistent through framse
        """
        ...
        # do the same thing with paddle 2
        if paddle_2_rect.top < 0:
            paddle_2_rect.top = 0
        if paddle_2_rect.bottom > SCREEN_HEIGHT:
            paddle_2_rect.bottom = SCREEN_HEIGHT

        # if the ball is getting close to the top (15 is an arbitrary number,
